...
_wrapStatus(const int Restarts, const int Threads, const Data *Pos_d, const int Cities) {
	...
	std::cout << "Blocks = " << Blocks << ", Threads  = " << Threads << ", TileSize = " << TileSize 
        << ", Status = " << getName(Status) << ", Shared Bytes = " << Shared_Bytes << ", Device Memory = " 
        << Device_Memory/(1024.0f*1024.0f) << "MB" << std::endl;
...
main(int argc, char *argv[]) {
	...
	std::cout << "Hours = " << hours << ", Minutes = " << minutes << ", Seconds = " << seconds << ", Milliseconds = " 
        << (int)(time) % 1000 << std::endl;
